...
//WL: here the pteditor.c is for hooking. So it will not be compiled as a km
...
static struct mm_struct *get_mm(size_t pid)
{
  ...
  if (pid != 0)
  {
    ...
    if (!vpid)
    {
      // printk("find vpid failed\n");
      return NULL;
    }
    task = pid_task(vpid, PIDTYPE_PID);
    if (!task)
    {
      // printk("get task failed\n");
      return NULL;
    }
  }
  if (task->mm)
...
//WL: the param "addr" is a virtual address
//WL: remove "static"
int resolve_vm(size_t addr, vm_t *entry, int lock)
{
  struct mm_struct *mm;
...
//WL: remove "static"
//WL: add debugging info
int update_vm(ptedit_entry_t *new_entry, int lock)
{
  vm_t old_entry;
  ...
  if ((old_entry.valid & PTEDIT_VALID_MASK_PTE) && (new_entry->valid & PTEDIT_VALID_MASK_PTE))
  {
    ...
    // printk("old_entry.pte: %p\n", old_entry.pte);
    // printk("value of old_entry.pte: %lx\n", *(old_entry.pte));

    // printk("new_entry->pte: %lx\n", (unsigned long)new_entry->pte);
    // printk("native_make_pte(new_entry->pte): %lx\n", native_make_pte(new_entry->pte));
    set_pte(old_entry.pte, native_make_pte(new_entry->pte));
    // printk("set_pte succeed\n");
  }

  //WL: need to init the following func pointer!
  invalidate_tlb(addr);
  // printk("invalidate_tlb succeed\n");
...
#endif
  // printk("up_write succeed\n");

  return 0;
}

//WL: remove "static"
void vm_to_user(ptedit_entry_t *user, vm_t *vm)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0)
...
//WL: the followings are the IOCTL
