...
impl Generate for Domain {
    fn generate(gen: &mut StdRng) -> Domain {
        const ADDRESS_FAMILIES: [c_int; 39] = [
            AF_DECnet,
            AF_ALG,
            AF_APPLETALK,
            AF_ASH,
        ...
        Domain(choose_one(gen, &ADDRESS_FAMILIES))
    }
}
...
#[cfg(not(feature = "type_only"))]
impl Generate for SockType {
    fn generate(gen: &mut StdRng) -> SockType {
        const SOCK_TYPES: [c_int; 9] = [
            SOCK_CLOEXEC,
            SOCK_DCCP,
            SOCK_DGRAM,
            SOCK_NONBLOCK,
        ...
        SockType(choose_one(gen, &SOCK_TYPES))
    }
}
...
/// Currently we only assume the protocol is zero
#[cfg(not(feature = "type_only"))]
impl Generate for Protocol {
    fn generate(_gen: &mut StdRng) -> Protocol {
        Protocol(0)
    }
}
#[cfg(feature = "type_only")]
...
#[cfg(not(feature = "type_only"))]
impl Generate for SockFd {
    fn generate(gen: &mut StdRng) -> SockFd {
        loop {
            let socket = Socket::generate(gen);
            match socket.call() {
                Ok(fd) => {
                    return SockFd(fd as _);
                }
                Err(_) => {
                    continue;
                }
...
#[cfg(not(feature = "type_only"))]
impl Generate for MsgFlag {
    fn generate(gen: &mut StdRng) -> MsgFlag {
        const MSG_FLAGS: [c_int; 18] = [
            MSG_CMSG_CLOEXEC,
            MSG_CONFIRM,
            MSG_CTRUNC,
            MSG_DONTROUTE,
        ...
        MsgFlag(choose_some_or(gen, &MSG_FLAGS))
    }
}
...
#[cfg(not(feature = "type_only"))]
impl Generate for Level {
    fn generate(gen: &mut StdRng) -> Level {
        const LEVELS: [c_int; 20] = [
            SOL_AAL,
            SOL_ALG,
            SOL_ATM,
            SOL_BLUETOOTH,
...
#[cfg(not(feature = "type_only"))]
impl Generate for OptName {
    fn generate(gen: &mut StdRng) -> Self {
        const OPTNAMES: [c_int; 62] = [
        ...
        OptName(choose_one(gen, &OPTNAMES))
    }
}
